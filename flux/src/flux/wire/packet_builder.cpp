...
namespace bcp::flux::wire
{
    ...
    // --- Content stage ---
    // A stage built from an error (Begin refused: no slot, spent, or the
    // target was never declared) holds an invalid writer, so a put must be
    // inert on a failed stage rather than reach into it. The chain already
    // keeps the first failure and reports it at the send; a later put adds
    // nothing, and touching the writer here would fault on the null slot.
