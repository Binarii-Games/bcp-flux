// What the protocol does on a link rather than what a packet costs. An
// in-process relay models one bottleneck per direction: arrival, a tail-drop
// queue bounded in bytes, serialisation at the configured rate, propagation
// delay, delivery. The bounded queue is the point. A rate limit alone cannot
// produce bufferbloat, and bufferbloat is what separates a controller that
// finds the bottleneck from one that fills it.
//
// One transfer crosses that link once per row: clean, under independent loss
// at one, two and five percent, and under Gilbert-Elliott loss at the same
// averages in bursts of twenty packets. Independent loss is what thermal
// noise looks like. Real links also lose in bursts, a fade or a handover
// takes everything for a stretch, and a burst defeats recovery that
// independent loss of the same average never troubles.
//
// The link is 50 Mbit with a 40 ms round trip and a 250 KB queue, one
// bandwidth-delay product. Timings are wall clock through a real scheduler,
// so treat small deltas as noise and compare against the printed floor, which
// is what a perfect sender would take at the link rate.
//
//   link_transfer_bench [MiB] [filter]
//
// MiB defaults to 100. A filter runs only the rows whose label contains it,
// so `link_transfer_bench 1024 clean` is one gigabyte on a clean link.
